...
- Smoke check (single check): start hub then `curl http://localhost:3456/api/health`
...
## Key Conventions
- CORAL syncs platform trigger URLs on startup. `python3 coral/update_config.py` is optional for immediate syncs.

...
Unified tool for real-time tracking of Spotify friend activity, profile changes, and playlist updates:
...
CONFIG_BLOCK = """
# === MONITORING MODE ===
# Select which monitoring mode to use:
#   friends - Monitor friend listening activity in real-time
#   profile - Monitor user profile, followers, and playlists changes
#   both    - Run both monitoring modes simultaneously (alternating checks)
...
# === FRIEND MONITORING SETTINGS ===
# These settings only apply when MONITORING_MODE = "friends" or "both"

# Whether to send an email when friend becomes active
# Can also be enabled via the -a flag
ACTIVE_NOTIFICATION = False

# Whether to send an email when friend goes inactive
...
# Whether to send an email when friend plays a song on loop
...
# How often to check for friend activity; in seconds
# Can also be set using the -c flag (when in friends mode)
FRIENDS_CHECK_INTERVAL = 30  # 30 seconds

# Time after which a friend is considered inactive (based on last activity); in seconds
...
# When enabled, songs played within the crossfade detection thresholds will be marked as crossfaded
...
# Interval for checking if a friend who disappeared from the list has reappeared; in seconds
...
# Can also be enabled via the --track-songs flag
TRACK_SONGS = False

# Method used to play songs in local Spotify client under macOS
# Methods: "apple-script" (recommended), "trigger-url"
SPOTIFY_MACOS_PLAYING_METHOD = "apple-script"

# Method used to play songs in local Spotify client under Linux
# Methods: "dbus-send" (most common), "qdbus", "trigger-url"
SPOTIFY_LINUX_PLAYING_METHOD = "dbus-send"

# Method used to play songs in local Spotify client under Windows
# Methods: "start-uri" (recommended), "spotify-cmd", "trigger-url"
...
# Spotify track ID to play when the friend goes offline (used when TRACK_SONGS is enabled)
# Leave empty to simply pause
SP_USER_GOT_OFFLINE_TRACK_ID = ""

# Delay before pausing the above track after friend goes offline; in seconds
SP_USER_GOT_OFFLINE_DELAY_BEFORE_PAUSE = 5

# Threshold to avoid false alarms when friend disappears from friend list
...
# Path to a file that is created when friend is active and deleted when inactive
...
# These settings only apply when MONITORING_MODE = "profile" or "both"
...
# Can also be set using the -c flag (when in profile mode)
PROFILE_CHECK_INTERVAL = 1800  # 30 mins
...
# Default check interval (will be overridden based on mode in main())
SPOTIFY_CHECK_INTERVAL = PROFILE_CHECK_INTERVAL  # Default to profile interval

LIVENESS_CHECK_COUNTER = LIVENESS_CHECK_INTERVAL / SPOTIFY_CHECK_INTERVAL if SPOTIFY_CHECK_INTERVAL > 0 else 0
...
# ============================================================
# FRIEND MONITORING FUNCTIONS
# ============================================================

# Friend monitoring functions extracted from spotify_monitor.py
# These need to be inserted into the merged spotify_monitor.py
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description=("Unified tool for monitoring Spotify friend activity, profile changes, and playlist updates [ https://github.com/misiektoja/spotify_monitor/ ]"), formatter_class=argparse.RawTextHelpFormatter
    )
    ...
    parser.add_argument(
        "--mode", "-m",
        dest="mode",
        choices=["friends", "profile", "both"],
        required=False,
        default=MONITORING_MODE,
        help="Monitoring mode: 'friends' (friend activity), 'profile' (user profile/playlists), or 'both' (simultaneous monitoring)"
    )
    ...
    parser.add_argument(
        ...
        help="Spotify user/friend URI ID to monitor",
        type=str
    )
    ...
    # Friend monitoring specific options (only apply when --mode friends or both)
    friend_opts = parser.add_argument_group("Friend monitoring options (--mode friends|both)")
    friend_opts.add_argument(
        "--notify-on-active",
        dest="active_notification",
        action="store_true",
        help="Send email when friend becomes active"
    )
    friend_opts.add_argument(
        "--notify-on-inactive",
        dest="inactive_notification",
        action="store_true",
        help="Send email when friend becomes inactive"
    )
    friend_opts.add_argument(
        "--notify-on-song",
        dest="song_notification",
        action="store_true",
        help="Send email on every song change"
    )
    friend_opts.add_argument(
        "--notify-on-track",
        dest="track_notification",
        action="store_true",
        help="Send email when specific monitored track plays"
    )
    friend_opts.add_argument(
        "--notify-on-loop",
        dest="song_on_loop_notification",
        action="store_true",
        help="Send email when friend plays a song on loop"
    )
    friend_opts.add_argument(
        "--track-songs",
        dest="track_songs",
        action="store_true",
        help="Auto-play friend's songs in your local Spotify client"
    )
    friend_opts.add_argument(
        "--inactivity-timeout",
        ...
        metavar="SECONDS",
        type=int,
        help="Time after which friend is considered inactive (default: 660 seconds / 11 mins)"
    )
    friend_opts.add_argument(
        "--monitored-tracks-file",
    ...
    friend_opts.add_argument(
        ...
        metavar="FLAG_FILE",
        type=str,
        help="Path to flag file (created when friend is active, deleted when inactive)"
    )
    friend_opts.add_argument(
        "--list-friends",
        dest="list_friends",
        action="store_true",
        help="List all friends and exit"
    )
    ...
    # Handle friend monitoring arguments
    if hasattr(args, 'active_notification') and args.active_notification:
        ACTIVE_NOTIFICATION = True
    
    if hasattr(args, 'inactive_notification') and args.inactive_notification:
        INACTIVE_NOTIFICATION = True
    
    if hasattr(args, 'song_notification') and args.song_notification:
        SONG_NOTIFICATION = True
    
    if hasattr(args, 'track_notification') and args.track_notification:
        TRACK_NOTIFICATION = True
    
    if hasattr(args, 'song_on_loop_notification') and args.song_on_loop_notification:
        SONG_ON_LOOP_NOTIFICATION = True
    
    if hasattr(args, 'track_songs') and args.track_songs:
        TRACK_SONGS = True
    
    if hasattr(args, 'inactivity_timeout') and args.inactivity_timeout:
        SPOTIFY_INACTIVITY_CHECK = args.inactivity_timeout
    
    if hasattr(args, 'monitor_list_file') and args.monitor_list_file:
        MONITOR_LIST_FILE = args.monitor_list_file
    
    if hasattr(args, 'flag_file') and args.flag_file:
    ...
    # Define signal handlers based on monitoring mode
    ...
    if platform.system() != 'Windows':
        ...
        if args.mode in ["friends", "both"]:
            # Friend monitoring signal handlers
            if args.mode == "friends":  # Only use these in pure friends mode to avoid conflicts
                ...
                signal.signal(signal.SIGWINCH, increase_inactivity_check_signal_handler)
                signal.signal(signal.SIGURG, decrease_inactivity_check_signal_handler)

    # Route to appropriate monitoring function based on mode
    if args.mode == "friends":
        # Friend monitoring mode
        tracks = []  # TODO: Load from MONITOR_LIST_FILE if specified
        if hasattr(args, 'list_friends') and args.list_friends:
            # TODO: Implement list friends functionality
            print("List friends functionality not yet implemented in merged version")
            sys.exit(0)
        SPOTIFY_CHECK_INTERVAL = FRIENDS_CHECK_INTERVAL
        spotify_monitor_friend_uri(args.user_id, tracks, CSV_FILE)
    elif args.mode == "profile":
        # Profile monitoring mode
        SPOTIFY_CHECK_INTERVAL = PROFILE_CHECK_INTERVAL
        spotify_profile_monitor_uri(args.user_id, CSV_FILE, playlists_to_skip)
    elif args.mode == "both":
        # Both monitoring modes - alternating implementation
        print("* Running both friend and profile monitoring in alternating mode")
        print("* Note: This is a simplified implementation - friend and profile checks will alternate")
        # TODO: Implement proper alternating or threaded monitoring
        # For now, default to profile mode
        SPOTIFY_CHECK_INTERVAL = min(FRIENDS_CHECK_INTERVAL, PROFILE_CHECK_INTERVAL)
        print(f"* Warning: 'both' mode not fully implemented yet, defaulting to profile monitoring")
        spotify_profile_monitor_uri(args.user_id, CSV_FILE, playlists_to_skip)

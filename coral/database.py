...
def init_db():
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_linked_profiles_person ON linked_profiles(person_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_linked_profiles_platform ON linked_profiles(platform_id)"
        )
        cursor.execute(
...
def unlink_profile(profile_id):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "SELECT person_id FROM linked_profiles WHERE id = ?", (profile_id,)
        )
        row = cursor.fetchone()
        if not row:
            return False

...
class CoralNotifier:
    ...
    def __init__(self, platform: str, hub_url: Optional[str] = None, 
                 secret: Optional[str] = None, auto_detect: bool = True):
        ...
        if auto_detect and CONFIG_AVAILABLE:
            ...
            if use_hub:
                ...
                self._secret = secret or config.get(f'{self.platform}.webhook.secret', '')
    ...
    def send_event(self, username: str, event_type: str, summary: str, 
                   event_time: Optional[str] = None, data: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            response = requests.post(self._hub_url, json=payload, headers=headers, timeout=5)
    ...
    def send_post_event(self, username: str, caption: str = "", post_url: str = "", 
                       post_id: str = "", timestamp: Optional[datetime] = None) -> bool:
        """Send a new post event"""
        return self.send_event(
            username=username, event_type='new_post',
    ...
    def send_bio_change(self, username: str, old_bio: str, new_bio: str) -> bool:
        ...
        return self.send_event(username=username, event_type='bio_change',
            summary="Bio updated", data={'old_bio': old_bio, 'new_bio': new_bio})
    
    def send_follower_change(self, username: str, old_count: int, new_count: int) -> bool:
        """Send a follower count change event"""
        diff = new_count - old_count
        return self.send_event(username=username, event_type='follower_change',
